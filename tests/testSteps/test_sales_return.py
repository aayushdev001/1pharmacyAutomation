...
class TestSalesReturn(BaseClass):
    def test_sales_return(self, driver, config):
        ...
        time.sleep(4)
        total_amount = billing_page.get_first_item_total()
        billing_discount = billing_page.get_first_discount()
        ...
        time.sleep(3)
        final_quantity = medicine_inventory_page.get_remaining_stock_strip(billing_page.get_first_default_batch())
