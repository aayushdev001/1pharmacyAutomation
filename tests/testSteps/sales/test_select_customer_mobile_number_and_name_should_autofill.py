...
class TestSelectMobileNumberAndNameShouldAutofill(BaseClass):
    def test_select_customer_mobile_number_and_name_should_autofill(self, driver, config):
        # report logging
        log = self.get_logger()
        ...
        billing_page.select_customer_mobile_number(config['customer_mobile_number'])
        time.sleep(2)
        assert billing_page.read_customer_name() == config['customer_name']
