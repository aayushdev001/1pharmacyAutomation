# changing the default payment type to card
...
class TestChangeDefaultPaymentType(BaseClass):
    def test_change_default_payment_type(self, driver, config):
        ...
        log.info(f"Default payment type = {billing_page.read_payment_type()}")
        billing_page.select_card_payment_type()
        log.info(f"New payment type = {billing_page.read_payment_type()}")
        ...
        assert bill_history_page.get_payment_type() == "Card"
